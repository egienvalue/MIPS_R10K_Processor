#!/bin/bash
rm -rf test_progs_out
rm -rf test_progs_diff_out
mkdir test_progs_out
mkdir test_progs_diff_out
...
for file in test_progs/*.s; do
    ...
    mv program.out ./test_progs_out/${filename}.program.out
	mv writeback.out ./test_progs_out/${filename}.writeback.out

    diff golden/"${filename}.writeback.out"	test_progs_out/${filename}.writeback.out > test_progs_diff_out/${filename}.wbdiffresult
    if test -s test_progs_diff_out/${filename}.wbdiffresult; then
    ...
    grep "@@@" test_progs_out/"${filename}.program.out" > newpr_trim
    diff goldenpr_trim newpr_trim > test_progs_diff_out/${filename}.prdiffresult
    if test -s test_progs_out/${filename}.prdiffresult; then
